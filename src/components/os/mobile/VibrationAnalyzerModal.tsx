...
function evaluateVibrationSeverity(
    ...
    const zoneLabels = {
        A: ft?.zoneGood || 'Zone A: Good',
        B: ft?.zoneSatisfactory || 'Zone B: Satisfactory',
        C: ft?.zoneUnsatisfactory || 'Zone C: Unsatisfactory',
        D: ft?.zoneUnacceptable || 'Zone D: Unacceptable',
    };
    const color = {
...
export function VibrationAnalyzerModal({ isOpen, onClose, ft: propFt, debugMode = false }: VibrationAnalyzerModalProps) {
    ...
    const [diagnosticResult, setDiagnosticResult] = useState({ zone: 'A', label: ft?.zoneGood || 'Zone A: Good', color: 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' });
