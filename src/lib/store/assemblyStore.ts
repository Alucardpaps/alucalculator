...
import { detectAndAssembleFasteners, AutoAssemblyResult } from '@/lib/engine/autoAssemblyEngine';
...
interface AssemblyActions {
  ...
  // ── AI Fastener Auto-Detection ──
  autoAssembleFasteners: () => AutoAssemblyResult;
...
export const useAssemblyStore = create<AssemblyState & AssemblyActions>()(
  (set, get) => ({
      ...
      autoAssembleFasteners: () => {
        const state = get();
        const result = detectAndAssembleFasteners(state.components);
        if (result.fastenersAdded > 0) {
          const updatedComponents = { ...state.components };
          for (const newComp of result.newComponents) {
            updatedComponents[newComp.id] = newComp;
            // Update connected components to reference the bolt
            for (const connectedId of newComp.connections) {
              if (updatedComponents[connectedId]) {
                const existingConns = updatedComponents[connectedId].connections;
                if (!existingConns.includes(newComp.id)) {
                  updatedComponents[connectedId] = {
                    ...updatedComponents[connectedId],
                    connections: [...existingConns, newComp.id],
                  };
                }
              }
            }
