...
export const metadata: Metadata = {
  title: 'Bearing Life Calculator (ISO 281 / L10h) | AluCalc OS',
  description:
    'Calculate rolling element bearing life (L10 rating life in millions of revolutions and L10h operating hours) according to ISO 281 and DIN ISO 281 standards.',
  alternates: {
    canonical: 'https://www.alucalculator.com/bearings/',
  },
};

export default function BearingsPage() {
  const seo = getModuleSeo('bearings');
  const method = getModuleMethod('bearings');

  return (
    <div className="module-page-frame min-h-screen flex flex-col justify-between bg-[#020408] text-white">
      <ModulePageSeoShell slug="bearings" seo={seo} />
      
      <div className="flex-1 w-full p-4 lg:p-8">
        <BearingsModule />
      </div>

      <SolverTransparencyDrawer solverId="bearings" />
