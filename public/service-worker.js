...
self.addEventListener('fetch', (event) => {
    ...
    if (url.pathname.includes('__next.')) {
        ...
        const prefix = '__next.';
        const nextIdx = rewrittenPathname.indexOf(prefix);
        
        if (nextIdx !== -1) {
            const before = rewrittenPathname.substring(0, nextIdx);
            const afterPrefix = rewrittenPathname.substring(nextIdx + prefix.length);
            
            if (afterPrefix.endsWith('.txt')) {
                const body = afterPrefix.substring(0, afterPrefix.length - 4);
                const newBody = body.replace(/\./g, '/');
                rewrittenPathname = before + prefix + newBody + '.txt';
            }
        }
