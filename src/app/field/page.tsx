...
export default function FieldToolsSuitePage() {
  ...
  return (
    <div className="w-full min-h-[calc(100vh-3.5rem)] pb-24 px-3 py-4 sm:px-6 max-w-4xl mx-auto space-y-4 select-none">
      ...
      {activeModal === 'limits-fits' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="w-full max-w-sm rounded-3xl border border-emerald-500/30 bg-[#070b14] p-5 shadow-2xl space-y-4 text-center">
            <h3 className="text-sm font-bold text-white">{tr ? 'Toleranslar (ISO 286)' : 'Limits & Fits (ISO 286)'}</h3>
            <p className="text-xs text-slate-400">{tr ? 'H7/g6, H7/p6 tolerans motoru.' : 'Shaft & hole fit solver.'}</p>
            <Link
              href="/fits-tolerances"
              className="block w-full py-3 rounded-xl bg-emerald-500 text-black font-bold font-mono text-xs uppercase"
            >
