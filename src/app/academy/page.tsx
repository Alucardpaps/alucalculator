...
import { AcademyGamificationHUD } from '@/components/academy/AcademyGamificationHUD';
import { DuolingoLearningPath } from '@/components/academy/DuolingoLearningPath';
...
type AcademyTab = 'duolingo' | 'curriculum' | 'calculators';
...
function AcademyPageContent() {
  ...
  const initialTab: AcademyTab = rawTab === 'calculators' ? 'calculators' : rawTab === 'curriculum' ? 'curriculum' : 'duolingo';
  ...
  useEffect(() => {
    ...
    const tab: AcademyTab = tabParam === 'calculators' ? 'calculators' : tabParam === 'curriculum' ? 'curriculum' : 'duolingo';
    setActiveTab(tab);
  }, [searchParams]);

  const switchTab = (tab: AcademyTab) => {
    setActiveTab(tab);
    const url = tab === 'duolingo' ? '/academy' : `/academy?tab=${tab}`;
  ...
  return (
    <div className="relative z-10 min-h-screen bg-transparent text-white selection:bg-blue-500/30 font-sans pb-32">
      {/* 🎮 Duolingo Gamification HUD Sticky Bar */}
      <AcademyGamificationHUD />

      {/* Toolbar: tabs + search + year filter */}
      <div className="sticky top-12 z-40 border-b border-white/5 backdrop-blur-3xl bg-black/60">
        <div className="max-w-7xl mx-auto px-6 py-3 space-y-3">
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
            <button
              type="button"
              onClick={() => switchTab('duolingo')}
            ...
            >
              <Route size={12} className="inline mr-1.5 -mt-0.5 text-emerald-400" />
              {isTr ? 'Yol Haritası (Duolingo)' : 'Learning Path'}
            </button>
            <button
              type="button"
      ...
      <main className={`mx-auto px-6 pt-8 space-y-16 ${activeTab === 'calculators' ? 'max-w-[1900px]' : 'max-w-7xl'}`}>
        {activeTab === 'duolingo' ? (
          <DuolingoLearningPath 
            departments={departments} 
            onSwitchToCurriculum={() => switchTab('curriculum')}
          />
        ) : activeTab === 'calculators' ? (
