...
import { ChevronDown, Menu, X, LayoutGrid, Ruler, Cog, Zap, Anchor, Flame, Layers, Droplets, Settings, ArrowRightLeft, Database, BookOpen, Mail, Newspaper, Calculator, Smartphone } from 'lucide-react';
...
export const Navbar = ({ lang, dict }: { lang: string, dict: any }) => {
    const modules = [
        ...
        { label: 'Mobile & Watch APK', icon: Smartphone, path: '/download' },
        ...
        <>
            ...
            <nav className={`w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-md border-b border-slate-200/50 shadow-sm py-3' : 'bg-transparent py-4 text-white'}`}>
                <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
                    ...
                    {/* Mobile Toggle */}
                    <button aria-label="Open Mobile Menu" className="md:hidden p-2 text-slate-800" onClick={() => setMenuOpen(!menuOpen)}>
                        {menuOpen ? <X /> : <Menu />}
                    </button>
                </div>
