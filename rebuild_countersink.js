...
// 1. REPLACEMENT OF UNDERCUT DATA WITH COUNTERSINK DATA
let topHalf = file.split('// DIN 76 Thread Undercuts')[0];
const newTopData = `// DIN 7991 Countersunk Head Screws (Havşa Başlı Civata)
const DIN_7991 = [
...
topHalf = topHalf + newTopData + file.split('// ============================================\n// MAIN COMPONENT')[1];

// Replace tab definition
topHalf = topHalf.replace(
    "{ id: 'undercut', label: 'Thread Undercuts', icon: Scissors }",
    "{ id: 'countersink', label: 'Countersink (Havşa)', icon: Scissors }"
);

// Replace component mounting
topHalf = topHalf.replace(
    "{activeTab === 'undercut' && <UndercutTab />}",
    "{activeTab === 'countersink' && <CountersinkTab />}"
);

// 2. REPLACEMENT OF UNDERCUT TAB WITH COUNTERSINK TAB
const splitTab = 'function UndercutTab() {';
const beforeUndercutTab = topHalf.split(splitTab)[0];
let afterUndercutTab = topHalf.split(splitTab)[1];

// We need to jump to "function ImbusSVG" to find the end of UndercutTab
const endOfTabs = afterUndercutTab.split('// SVG HELPER COMPONENTS')[1];

const newCountersinkTab = `function CountersinkTab() {
...
// ============================================
// SVG HELPER COMPONENTS` + endOfTabs;

let finalTop = beforeUndercutTab + newCountersinkTab;

// 3. REPLACEMENT OF ALL SVGS FOR BETTER SCALING
// Strip away the existing SVG components
const beforeSVGs = finalTop.split('function ImbusSVG({ data }: { data: any }) {')[0];

const newSVGs = `function ImbusSVG({ data }: { data: any }) {
...
fs.writeFileSync('src/components/modules/mechanical/MachiningDetailsModule.tsx', finalTop + newSVGs);
console.log('Update successful: replaced undercuts with countersinks and fixed circlip/keyway scales.');
