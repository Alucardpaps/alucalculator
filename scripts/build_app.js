...
buildApp().catch(err => {
  console.warn('[Build App] Android compilation skipped or failed:', err.message);
  console.log('[Build App] Falling back to exporting existing pre-built assets.');
  ...
  const apkFile = path.join(publicAppDir, 'alucalc-release.apk');
  const aabFile = path.join(publicAppDir, 'alucalc-release.aab');
  const wearApkFile = path.join(publicAppDir, 'alucalc-wear-release.apk');
  const wearAabFile = path.join(publicAppDir, 'alucalc-wear-release.aab');
  
  if (fs.existsSync(apkFile)) fs.copyFileSync(apkFile, path.join(outAppDir, 'alucalc-release.apk'));
  if (fs.existsSync(aabFile)) fs.copyFileSync(aabFile, path.join(outAppDir, 'alucalc-release.aab'));
  if (fs.existsSync(wearApkFile)) fs.copyFileSync(wearApkFile, path.join(outAppDir, 'alucalc-wear-release.apk'));
  if (fs.existsSync(wearAabFile)) fs.copyFileSync(wearAabFile, path.join(outAppDir, 'alucalc-wear-release.aab'));
  
  console.log('[Build App] Static build assets (Phone & Wear OS APKs) exported successfully.');
  process.exit(0);
});
