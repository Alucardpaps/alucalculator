...
export const BASE_REGISTRY = {
    ...
    'matrix-screensaver': {
        ...
        title: 'Matrix Terminal Screensaver',
        category: 'software',
        iconName: 'Terminal',
...
export const MODULE_ALIASES: Record<string, string> = {
    ...
    'gear-module': 'gears-bearings',
    'gear-ratio': 'gears-bearings',
    'gear-contact-stress': 'gears-bearings',
...
calculatorsDB.forEach((calc: any) => {
    // Skip duplicate aliases of existing core workstation modules
    if (MODULE_ALIASES[calc.slug]) return;

    if (!BASE_REGISTRY[calc.slug]) {
        const cleanTitle = (calc.title || '')
            .replace(/\s*Calculator\s*&\s*(?:Engineering\s*)?Guide\s*-\s*AluCalc.*$/i, '')
            .replace(/\s*Calculator\s*&\s*Guide\s*-\s*AluCalc.*$/i, '')
            .trim();

        let category: ModuleCategory = (calc.category as ModuleCategory) || 'science';
        if (calc.category === 'structures') category = 'civil';
        if (calc.category === 'aerospace') category = 'mechanical';
        if (calc.category === 'fluids') category = 'mechanical';
        if (calc.category === 'fasteners') category = 'mechanical';
        if (calc.category === 'thermal') category = 'mechanical';

        BASE_REGISTRY[calc.slug] = {
            type: calc.slug as any,
            title: cleanTitle || calc.slug.replace(/-/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()),
            category: category,
            iconName: 'Calculator',
...
/**
 * SINGLE SOURCE OF TRUTH FOR CALCULATOR & MODULE COUNTS
 * Computed deterministically at build time from the unified registry.
 */
export const TOTAL_MODULES = Object.keys(MODULE_REGISTRY).length;
export const TOTAL_CALCULATORS_COUNT = TOTAL_MODULES;
export const TOTAL_CALCULATORS_LABEL = `${Math.floor(TOTAL_MODULES / 10) * 10}+`;
