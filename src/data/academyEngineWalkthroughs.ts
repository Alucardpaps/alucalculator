export type EngineWalkthroughStep = {
  title: string;
  detail: string;
  formula?: string;
};

export type AcademyEngineWalkthrough = {
  engineName: string;
  sourceFile: string;
  inputs: string[];
  outputs: string[];
  steps: EngineWalkthroughStep[];
};

export const ACADEMY_ENGINE_WALKTHROUGHS: Record<string, AcademyEngineWalkthrough> = {
...
export function getEngineWalkthrough(slug: string): AcademyEngineWalkthrough | undefined {
  return ACADEMY_ENGINE_WALKTHROUGHS[slug];
}

/** Curriculum order for prev/next navigation. */
export const ACADEMY_LESSON_ORDER: string[] = [
  'engineering-units-and-standards',
  'fundamentals-of-statics',
  'introduction-to-machine-elements',
  'thread-geometry-standards',
...
export function getAdjacentLessons(slug: string): { prev?: string; next?: string } {
  const idx = ACADEMY_LESSON_ORDER.indexOf(slug);
  if (idx === -1) return {};
  return {
