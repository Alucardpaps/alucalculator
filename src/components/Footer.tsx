"use client";

import { Linkedin, Mail, AlertTriangle } from 'lucide-react';
import { getSiteFooterStrings } from '@/locales/siteFooterTranslations';

export const Footer = ({ lang, dict }: { lang: string, dict: any }) => {
    const f = getSiteFooterStrings(lang);
    return (
        <footer className="bg-slate-900 border-t border-slate-800 py-12 text-slate-400">
            <div className="max-w-7xl mx-auto px-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">

                    {/* Developer Info */}
                    <div className="flex flex-col gap-4">
                        <h3 className="text-lg font-bold text-white mb-2">Developed by</h3>
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold text-xl">
                                AY
                            </div>
                            <div>
                                <div className="font-bold text-white text-lg">Abdulsamet Yildirim</div>
                                <div className="text-xs text-slate-500 uppercase tracking-widest font-bold">Frontend Architect & Engineer</div>
                            </div>
                        </div>

                        <div className="flex flex-wrap gap-2.5 mt-2">
                            <a
                                href="/app/alucalc-release.apk"
                                download="alucalc-release.apk"
                                className="px-3 py-1.5 bg-cyan-950/60 hover:bg-cyan-900 border border-cyan-800/50 text-cyan-300 text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5"
                            >
                                <span>📱 Android APK</span>
                            </a>
                            <a
                                href="/app/alucalc-wear-release.apk"
                                download="alucalc-wear-release.apk"
                                className="px-3 py-1.5 bg-sky-950/60 hover:bg-sky-900 border border-sky-800/50 text-sky-300 text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5"
                            >
                                <span>⌚ Wear OS APK</span>
                            </a>
                            <a
                                href="https://www.linkedin.com/in/abdulsamet-yildirim-b353b718a/?locale=en"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="p-2 bg-slate-800 hover:bg-blue-600 text-white rounded-lg transition-colors"
                            >
                                <Linkedin size={18} />
                            </a>
                            <a
                                href="mailto:contact@alucalculator.com"
                                className="p-2 bg-slate-800 hover:bg-red-500 text-white rounded-lg transition-colors"
                            >
                                <Mail size={18} />
                            </a>
                        </div>
                    </div>

                    {/* Disclaimer */}
                    <div className="bg-amber-900/20 border border-amber-900/50 p-6 rounded-2xl">
                        <div className="flex items-start gap-3">
                            <AlertTriangle className="text-amber-500 flex-shrink-0 mt-1" size={24} />
                            <div>
                                <h4 className="text-white font-bold mb-1">{f.disclaimerStable}</h4>
                                <p className="text-sm text-slate-400 leading-relaxed">
                                    {f.disclaimerText}
                                </p>
                            </div>
                        </div>
                    </div>

                </div>

                <div className="mt-12 pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center text-xs text-slate-600 gap-2">
                    <p>&copy; 2026 AluCalculator. All rights reserved.</p>
                    <div className="flex items-center gap-4">
                        <a href="/privacy" className="text-slate-500 hover:text-cyan-400 transition-colors">Privacy & KVKK</a>
                        <p>Designed with Intentional Minimalism.</p>
                    </div>
                </div>
            </div>
        </footer>
    );
};
