...
import { X, Volume2, Play, Square, Settings, RefreshCw } from 'lucide-react';
...
function evaluateNoiseSeverity(db: number, ft?: FieldToolsStrings) {
    if (db < 80) {
        return {
            zone: 'SAFE',
...
export function SoundMeterModal({ isOpen, onClose, ft: propFt, debugMode = false }: SoundMeterModalProps) {
    ...
    const startAudioAnalysis = (stream: MediaStream) => {
        ...
        try {
            ...
            const drawLoop = () => {
                ...
                // Map RMS to decibels (approximated based on mic reference levels)
                ...
                // Get Frequency Data for rendering spectrum
    ...
    const startSimulatedAnalysis = () => {
        ...
        const simLoop = () => {
            ...
            // Create fake FFT frequency coefficients
            const freqData = new Uint8Array(128);
            const time = Date.now() / 1000;
            for (let i = 0; i < 128; i++) {
                // Generate a spectrum with peaks corresponding to simulated gear/motor whines or exhaust hums
                let factor = 0;
                if (simSource === 'office') {
    ...
    const drawSpectrum = (freqData: Uint8Array) => {
        ...
        for (let i = 0; i < freqData.length; i++) {
            ...
            // Heatmap color: Cyan -> Yellow -> Magenta based on intensity
            const intensity = freqData[i] / 255;
            let barColor = 'rgba(6, 182, 212, 0.7)'; // Cyan
            if (intensity > 0.8) {
                barColor = 'rgba(236, 72, 153, 0.8)'; // Pink/Magenta
            } else if (intensity > 0.5) {
                barColor = 'rgba(234, 179, 8, 0.75)'; // Yellow
            }
        ...
        // Draw rolling history line on top
                    ...
                    <div className="w-full max-w-lg mx-auto bg-[#070b12] border-t md:border border-cyan-900/30 rounded-t-[2.5rem] md:rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[90vh] md:h-auto max-h-[85vh]">
                        ...
                        <header className="flex-none p-5 border-b border-white/5 flex items-center justify-between bg-black/20">
                            <div>
                                ...
                                <p className="text-[10px] text-slate-500 font-mono mt-0.5">{ft.soundMeterDesc || 'Real-time ambient noise analyzer'}</p>
                            </div>
                            <button 
                        ...
                        <div className="flex-none p-4 bg-black flex flex-col items-center justify-center relative">
                            <canvas 
                                ...
                                width={480}
                                height={160}
                                className="w-full h-[140px] bg-slate-950/60 rounded-2xl border border-white/5 shadow-inner"
                            />
                            
                            <div className="absolute top-6 left-6 text-[8px] font-mono text-cyan-400/50 uppercase select-none">
                                FFT Frequency Spectrum (125Hz - 8kHz)
                            </div>
                        ...
                        {/* Telemetry numbers & compliance */}
                        <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
                            
                            {/* Big decibel dials */}
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-4 bg-slate-950/30 border border-white/5 rounded-2xl text-center space-y-1 relative group overflow-hidden">
                                    ...
                                    <span className="text-[8px] font-mono text-slate-500 uppercase tracking-wider block">{ft.soundIntensityDb || 'Sound Intensity'}</span>
                                ...
                                <div className="p-4 bg-slate-950/30 border border-white/5 rounded-2xl text-center space-y-1 relative overflow-hidden">
                                    <span className="text-[8px] font-mono text-slate-500 uppercase tracking-wider block">{ft.peakDb || 'Peak level'}</span>
                                    <div className="flex items-baseline justify-center gap-1.5">
                                        ...
                                        <button 
                                            ...
                                            className="p-1 rounded bg-white/5 text-slate-500 hover:text-white transition-all cursor-pointer"
                                            title="Reset Peak"
                                        >
                                            <RefreshCw size={10} />
                                        </button>
                            ...
                            {/* OSHA safety classification card */}
                            <div className={`p-4 rounded-2xl border transition-all duration-300 flex items-center justify-between ${severity.color}`}>
                                <div className="space-y-1">
                                    <span className="text-[8px] font-mono font-bold block uppercase tracking-wider">{ft.noiseSeverityOSHA || 'OSHA Noise Compliance'}</span>
                                ...
                                <div className="p-2.5 rounded-xl bg-black/40 border border-white/5 text-center font-mono font-bold text-[9px] shrink-0 text-cyan-400 select-none">
                                    OSHA H.1
                                </div>
                            </div>
                                ...
                                <div className="p-4 bg-slate-950/60 border border-yellow-500/10 rounded-2xl space-y-3">
                                    <div className="flex items-center justify-between pb-1 border-b border-white/5">
                                        <span className="text-[8px] font-mono font-bold text-yellow-500 uppercase tracking-wider flex items-center gap-1">
                                            <Settings size={10} />
                                            {ft.noiseSimulator || 'Noise Simulator (Sandbox)'}
                                        </span>
                                        <span className="text-[8px] font-mono text-slate-500">SIMULATING</span>
                                    </div>
                                    <div className="space-y-1 text-[10px] font-mono">
                                        <label className="text-slate-500 block text-[8px] uppercase tracking-wider">{ft.noiseSource || 'Select Noise Source'}</label>
                                        <select 
                                            value={simSource} 
                                            onChange={e => {
                                                playClickSound();
                                            ...
                                            className="w-full bg-[#03060a] border border-cyan-900/30 rounded-lg p-2.5 text-white text-xs outline-none focus:border-cyan-500 transition-all cursor-pointer"
                                        >
                        ...
                        <div className="flex-none p-5 bg-slate-950/40 border-t border-white/5 flex gap-3">
                            {micPermission === 'default' && !isSimulated && (
                                <button
                                    ...
                                    className="flex-1 py-3.5 bg-yellow-500 text-black font-extrabold text-xs uppercase tracking-widest rounded-xl hover:bg-yellow-400 transition-all active:scale-95 shadow-[0_0_15px_rgba(234,179,8,0.2)] cursor-pointer"
                                >
