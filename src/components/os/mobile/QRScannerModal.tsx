...
export function QRScannerModal({ isOpen, onClose, onAddPart, ft: propFt, language: propLang, debugMode = false }: QRScannerModalProps) {
    const { language: storeLang } = useI18nStore();
    const language = propLang || storeLang;
    const ft = propFt || getFieldToolsStrings(language as any);
    ...
    const handleAddToBOM = () => {
        ...
        if (onAddPart) onAddPart(scannedItem);
        onClose();
    };
